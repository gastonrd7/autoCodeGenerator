...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.instagramPrintCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.instagramPrintCount), function(err, msg){
            var hand = new instagramPrintCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.instagramPrintCount);
        ...
        {
            return Promise.reject("Error in campaign instagramPrintCount. Error: " + JSON.stringify(err));
        }
    }    
}
