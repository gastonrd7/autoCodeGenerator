...
export default class instagramFemaleInfluencedCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.instagramFemaleInfluencedCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.REMOVED), function(err, msg){
            var hand = new instagramFemaleInfluencedCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in campaign instagramFemaleInfluencedCount. Error: " + JSON.stringify(err));
        }
    }    
}
