...
export default class engagementMaleInfluencer extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.engagementMaleInfluencer, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.REMOVED), function(err, msg){
            var hand = new engagementMaleInfluencer();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in campaign engagementMaleInfluencer. Error: " + JSON.stringify(err));
        }
    }    
}
