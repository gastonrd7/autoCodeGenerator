...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.twitterTagCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.twitterTagCount), function(err, msg){
            var hand = new twitterTagCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.twitterTagCount);
        ...
        {
            return Promise.reject("Error in campaign twitterTagCount. Error: " + JSON.stringify(err));
        }
    }    
}
