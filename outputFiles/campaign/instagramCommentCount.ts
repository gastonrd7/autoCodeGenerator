...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.instagramCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.instagramCommentCount), function(err, msg){
            var hand = new instagramCommentCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.instagramCommentCount);
        ...
        {
            return Promise.reject("Error in campaign instagramCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
