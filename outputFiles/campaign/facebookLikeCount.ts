...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.facebookLikeCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.facebookLikeCount), function(err, msg){
            var hand = new facebookLikeCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.facebookLikeCount);
        ...
        {
            return Promise.reject("Error in campaign facebookLikeCount. Error: " + JSON.stringify(err));
        }
    }    
}
