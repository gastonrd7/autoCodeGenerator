...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.twitterCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.twitterCommentCount), function(err, msg){
            var hand = new twitterCommentCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.twitterCommentCount);
        ...
        {
            return Promise.reject("Error in campaign twitterCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
