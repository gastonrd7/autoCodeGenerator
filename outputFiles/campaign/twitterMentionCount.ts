...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.twitterMentionCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.twitterMentionCount), function(err, msg){
            var hand = new twitterMentionCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.twitterMentionCount);
        ...
        {
            return Promise.reject("Error in campaign twitterMentionCount. Error: " + JSON.stringify(err));
        }
    }    
}
