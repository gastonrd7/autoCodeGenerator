...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.instagramTagCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.instagramTagCount), function(err, msg){
            var hand = new instagramTagCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.instagramTagCount);
        ...
        {
            return Promise.reject("Error in campaign instagramTagCount. Error: " + JSON.stringify(err));
        }
    }    
}
