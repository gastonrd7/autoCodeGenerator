...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.instagramMentionCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.instagramMentionCount), function(err, msg){
            var hand = new instagramMentionCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.instagramMentionCount);
        ...
        {
            return Promise.reject("Error in campaign instagramMentionCount. Error: " + JSON.stringify(err));
        }
    }    
}
