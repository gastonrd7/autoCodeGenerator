...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.twitterHashtagCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.twitterHashtagCount), function(err, msg){
            var hand = new twitterHashtagCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.twitterHashtagCount);
        ...
        {
            return Promise.reject("Error in campaign twitterHashtagCount. Error: " + JSON.stringify(err));
        }
    }    
}
