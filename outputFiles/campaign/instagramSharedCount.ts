...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.instagramSharedCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.instagramSharedCount), function(err, msg){
            var hand = new instagramSharedCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.instagramSharedCount);
        ...
        {
            return Promise.reject("Error in campaign instagramSharedCount. Error: " + JSON.stringify(err));
        }
    }    
}
