...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.twitterLinkCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.twitterLinkCount), function(err, msg){
            var hand = new twitterLinkCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.twitterLinkCount);
        ...
        {
            return Promise.reject("Error in campaign twitterLinkCount. Error: " + JSON.stringify(err));
        }
    }    
}
