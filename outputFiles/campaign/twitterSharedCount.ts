...
{
    constructor(){
        super(globalModels.Model.campaign, globalModels.campaignFields.twitterSharedCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.twitterSharedCount), function(err, msg){
            var hand = new twitterSharedCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.advertisement, null, 
                {[globalModels.advertisementFields.campaignId]: msgPayload[globalModels.advertisementFields.campaignId]},
                 null, null, null, null, globalModels.advertisementFields.twitterSharedCount);
        ...
        {
            return Promise.reject("Error in campaign twitterSharedCount. Error: " + JSON.stringify(err));
        }
    }    
}
