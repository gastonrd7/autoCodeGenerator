...
export default class twitterSharedCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.post, globalModels.postFields.twitterSharedCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.CREATED), function(err, msg){
            var hand = new twitterSharedCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in post twitterSharedCount. Error: " + JSON.stringify(err));
        }
    }    
}
