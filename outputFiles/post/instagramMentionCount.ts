...
export default class instagramMentionCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.post, globalModels.postFields.instagramMentionCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.CREATED), function(err, msg){
            var hand = new instagramMentionCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            if (msgPayload[globalModels.insightFields.type] != globalModels.insightTypeEnum.Mention) return Promise.resolve({proceed: false, newValue: null});
            if (msgPayload[globalModels.insightFields.platform] != globalModels.insightPlatformEnum.Instagram) return Promise.resolve({proceed: false, newValue: null});
            ...
            await request.init(globalModels.Model.insight, [globalModels.insightFields.creationDt], 
            {
                ...
                [globalModels.insightFields.type]: globalModels.insightTypeEnum.Mention,
                [globalModels.insightFields.platform]: globalModels.insightPlatformEnum.Instagram
            }, 
        ...
        {
            return Promise.reject("Error in post instagramMentionCount. Error: " + JSON.stringify(err));
        }
    }    
}
