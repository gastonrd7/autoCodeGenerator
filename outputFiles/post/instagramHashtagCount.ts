...
export default class instagramHashtagCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.post, globalModels.postFields.instagramHashtagCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.CREATED), function(err, msg){
            var hand = new instagramHashtagCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            if (msgPayload[globalModels.insightFields.type] != globalModels.insightTypeEnum.Hashtag) return Promise.resolve({proceed: false, newValue: null});
            ...
            await request.init(globalModels.Model.insight, [globalModels.insightFields.creationDt], 
            {
                ...
                [globalModels.insightFields.type]: globalModels.insightTypeEnum.Hashtag,
        ...
        {
            return Promise.reject("Error in post instagramHashtagCount. Error: " + JSON.stringify(err));
        }
    }    
}
