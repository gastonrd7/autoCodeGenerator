...
export default class instagramTagCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.post, globalModels.postFields.instagramTagCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.CREATED), function(err, msg){
            var hand = new instagramTagCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            if (msgPayload[globalModels.insightFields.type] != globalModels.insightTypeEnum.Tag) return Promise.resolve({proceed: false, newValue: null});
            ...
            await request.init(globalModels.Model.insight, [globalModels.insightFields.creationDt], 
            {
                ...
                [globalModels.insightFields.type]: globalModels.insightTypeEnum.Tag,
        ...
        {
            return Promise.reject("Error in post instagramTagCount. Error: " + JSON.stringify(err));
        }
    }    
}
