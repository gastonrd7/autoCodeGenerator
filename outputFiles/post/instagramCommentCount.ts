...
export default class instagramCommentCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.post, globalModels.postFields.instagramCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.CREATED), function(err, msg){
            var hand = new instagramCommentCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            if (msgPayload[globalModels.insightFields.type] != globalModels.insightTypeEnum.Comment) return Promise.resolve({proceed: false, newValue: null});
            ...
            await request.init(globalModels.Model.insight, [globalModels.insightFields.creationDt], 
            {
                ...
                [globalModels.insightFields.type]: globalModels.insightTypeEnum.Comment,
        ...
        {
            return Promise.reject("Error in post instagramCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
