...
export default class instagramSharedCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.post, globalModels.postFields.instagramSharedCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.insight, globalEvents.Event.CREATED), function(err, msg){
            var hand = new instagramSharedCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            if (msgPayload[globalModels.insightFields.type] != globalModels.insightTypeEnum.Shared) return Promise.resolve({proceed: false, newValue: null});
            ...
            await request.init(globalModels.Model.insight, [globalModels.insightFields.creationDt], 
            {
                ...
                [globalModels.insightFields.type]: globalModels.insightTypeEnum.Shared,
        ...
        {
            return Promise.reject("Error in post instagramSharedCount. Error: " + JSON.stringify(err));
        }
    }    
}
