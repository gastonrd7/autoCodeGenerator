...
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.notSponsoredCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.notSponsoredCommentCount), function(err, msg){
            var hand = new notSponsoredCommentCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.notSponsoredCommentCount);
        ...
        {
            return Promise.reject("Error in advertisement notSponsoredCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
