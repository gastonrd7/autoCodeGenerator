...
export default class notSponsoredPrintCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.notSponsoredPrintCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.CREATED), function(err, msg){
            var hand = new notSponsoredPrintCount();
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.notSponsoredPrintCount), function(err, msg){
            var hand = new notSponsoredPrintCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.notSponsoredPrintCount);
        ...
        {
            return Promise.reject("Error in advertisement notSponsoredPrintCount. Error: " + JSON.stringify(err));
        }
    }    
}
