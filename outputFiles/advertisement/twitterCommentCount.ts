...
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.twitterCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.twitterCommentCount), function(err, msg){
            var hand = new twitterCommentCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.twitterCommentCount);
        ...
        {
            return Promise.reject("Error in advertisement twitterCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
