...
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.instagramTagCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.instagramTagCount), function(err, msg){
            var hand = new instagramTagCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.instagramTagCount);
        ...
        {
            return Promise.reject("Error in advertisement instagramTagCount. Error: " + JSON.stringify(err));
        }
    }    
}
