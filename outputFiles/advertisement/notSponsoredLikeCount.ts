...
export default class notSponsoredLikeCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.notSponsoredLikeCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.CREATED), function(err, msg){
            var hand = new notSponsoredLikeCount();
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.notSponsoredLikeCount), function(err, msg){
            var hand = new notSponsoredLikeCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.notSponsoredLikeCount);
        ...
        {
            return Promise.reject("Error in advertisement notSponsoredLikeCount. Error: " + JSON.stringify(err));
        }
    }    
}
