...
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.instagramCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.instagramCommentCount), function(err, msg){
            var hand = new instagramCommentCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.instagramCommentCount);
        ...
        {
            return Promise.reject("Error in advertisement instagramCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
