...
export default class zeroBudgetDt extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.zeroBudgetDt, null);
    }
    async subcribeEvents(){
        
        
        
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.advertisement, globalEvents.Event.UPDATED, globalModels.advertisementFields.moneyAvailable), function(err, msg){
            var hand = new zeroBudgetDt();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in advertisement zeroBudgetDt. Error: " + JSON.stringify(err));
        }
    }    
}
