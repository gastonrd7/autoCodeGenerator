...
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.facebookCommentCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.facebookCommentCount), function(err, msg){
            var hand = new facebookCommentCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.facebookCommentCount);
        ...
        {
            return Promise.reject("Error in advertisement facebookCommentCount. Error: " + JSON.stringify(err));
        }
    }    
}
