...
{
    constructor(){
        super(globalModels.Model.advertisement, globalModels.advertisementFields.twitterMentionCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.post, globalEvents.Event.UPDATED, globalModels.postFields.twitterMentionCount), function(err, msg){
            var hand = new twitterMentionCount();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {          
            ...
            await request.init(globalModels.Model.post, null, 
                {[globalModels.postFields.advertisementId]: msgPayload[globalModels.postFields.advertisementId]},
                 null, null, null, null, globalModels.postFields.twitterMentionCount);
        ...
        {
            return Promise.reject("Error in advertisement twitterMentionCount. Error: " + JSON.stringify(err));
        }
    }    
}
