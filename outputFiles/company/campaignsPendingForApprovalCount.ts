...
export default class campaignsPendingForApprovalCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.company, globalModels.companyFields.campaignsPendingForApprovalCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.campaign, globalEvents.Event.CREATED), function(err, msg){
            var hand = new campaignsPendingForApprovalCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in company campaignsPendingForApprovalCount. Error: " + JSON.stringify(err));
        }
    }    
}
