...
export default class campaignsOnGoingCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.company, globalModels.companyFields.campaignsOnGoingCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.campaign, globalEvents.Event.CREATED), function(err, msg){
            var hand = new campaignsOnGoingCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in company campaignsOnGoingCount. Error: " + JSON.stringify(err));
        }
    }    
}
