...
export default class campaignsDraftCount extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.company, globalModels.companyFields.campaignsDraftCount, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.campaign, globalEvents.Event.CREATED), function(err, msg){
            var hand = new campaignsDraftCount();
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in company campaignsDraftCount. Error: " + JSON.stringify(err));
        }
    }    
}
