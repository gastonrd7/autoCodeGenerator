...
{
    constructor(){
        super(globalModels.Model.company, globalModels.companyFields.budgetAvailable, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.campaign, globalEvents.Event.UPDATED, globalModels.campaignFields.budgetAvailable), function(err, msg){
            var hand = new budgetAvailable();
            hand.listenerExecutor(err, msg);
        });
