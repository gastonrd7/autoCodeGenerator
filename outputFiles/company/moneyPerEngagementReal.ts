...
export default class moneyPerEngagementReal extends abstractFieldHandler
{
    constructor(){
        super(globalModels.Model.company, globalModels.companyFields.moneyPerEngagementReal, null);
    }
    async subcribeEvents(){
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.campaign, globalEvents.Event.CREATED), function(err, msg){
            var hand = new moneyPerEngagementReal();
        ...
        await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.campaign, globalEvents.Event.UPDATED, globalModels.campaignFields.moneyPerEngagementReal), function(err, msg){
            var hand = new moneyPerEngagementReal();
            hand.listenerExecutor(err, msg);
        });
    ...
    async resolveScalar(msgPayload): Promise<{proceed, newValue}> {
        ...
        {
            return Promise.reject("Error in company moneyPerEngagementReal. Error: " + JSON.stringify(err));
        }
    }    
}
