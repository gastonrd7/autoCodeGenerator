...
{
    constructor(){
        super("_onCreation_fill_extra_fields", globalModels.Model.company);
    }
    async subcribeEvents(){
        
            await messagingService.subscribe(this.name, await eventFormater.formatEvent(globalModels.Model.company, globalEvents.Event.CREATED), function(err, msg){
    ...
    async resolvenewValue(msgPayload: any): Promise<{ proceed: boolean; newValues: any; }> {
        ...
        {
            return Promise.reject("Error in company _onCreation_fill_extra_fields. Error: " + JSON.stringify(err));
        }
    }    
}
